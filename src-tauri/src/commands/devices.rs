...
pub fn list_devices() -> Result<serde_json::Value, String> {
    let conn = db_conn()?;
    let mut stmt = conn
        .prepare("SELECT id, name, description, notes, last_connected_at, created_at, updated_at FROM device ORDER BY id ASC")
        .map_err(|e| e.to_string())?;
    let rows = stmt
        .query_map([], |row| {
            Ok(serde_json::json!({
                ...
                "lastConnectedAt": row.get::<_, Option<i64>>("last_connected_at")?,
                "createdAt": row.get::<_, Option<i64>>("created_at")?,
                "updatedAt": row.get::<_, Option<i64>>("updated_at")?,
            }))
        })
        .map_err(|e| e.to_string())?;
    let mut v = Vec::new();
